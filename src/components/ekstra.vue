    ...
    <header>
      <nav class="navbar">
        <div style="width: 60%">
          ...
          <div class="navbar-brand">Menu Pendaftaran Ekstra |</div>
        </div>
        <br>
      ...
      <table class="table table-striped table-bordered table-hover">
        ...
        <tbody>
          ...
          <tr :key="x" v-for="x in cariData" v-if="x.kelas == kelas || kelas == ''">
            ...
            <td>
              <input type="checkbox" v-model="x.robotik">
            </td>
            <td>
              <input type="checkbox" v-model="x.futsal">
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  filters: {
    capitalize: function(value) {
      ...
      return value.charAt(0).toUpperCase() + value.slice(1); //tanpa parameter satu sampai akhir - non mutating
    }
  },
  ...
  created() {
    axios.get("http://localhost:3000/api/extra").then(res => {
      this.extra = res.data;
      console.log(res.data);
      console.log(this.extra[1].nama_ekstra);
    });
